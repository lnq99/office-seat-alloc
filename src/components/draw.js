...
Entity.prototype = {
  ...
  respondToCollision: function (other, response) {
    ...
    if (this.isSolid && other.isSolid && !(this.isHeavy && other.isHeavy)) {
      if (this.isHeavy) {
...
World.prototype = {
  addEntity: function (data, options) {
    var entity = new Entity(data, this.canvas.path(), options)
    // Make the display item draggable if requested.
    if (options.draggable) {
      entity.display.drag(
        moveDrag(entity, this),
        startDrag(entity),
        endDrag(entity)
      )
    }
...
function draw(w, h, rects) {
  var canvas = Raphael('draw', w, h)
  var world = new World(canvas)

  for (let rect of rects) {
    console.log(rect)
    world.addEntity(B(V(rect[0], rect[1]), 50, 40).setAngle(rect[2]), {
      solid: true,
      draggable: true
      // heavy: true
    })
